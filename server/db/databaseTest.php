...
class Database
{
    ...
    // Подключаемся к БД MongoDB + сохраняем подкл в $client и выбираем базу
...
$testStCollection = Database::getCollection('testSt');
...
// Проверяем, существует ли коллекция sites
if (!Database::collectionExists('sites')) {
    echo "Коллекция sites не существует, создаем и заполняем...\n";
...
//         $char = chr(hexdec('03')); // получам символ с кодом 0x03 ETX сигнал конца текста
